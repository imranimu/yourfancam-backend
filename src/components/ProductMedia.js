...
const ProductMedia = ({ image, title, description, right, status, ...restProps }) => {
  return (
    <Media {...restProps}>
      <Media left>
        <Media
      ...
      <Media body className="overflow-hidden">
        <Media heading tag="h5" className="text-truncate">
          {title} {status && typeof status === 'string' ? (
                    <Typography className="text-capitalize text-success btn mb-0">{status}</Typography>
                  ) : (
                    status
                  )}
        </Media>
